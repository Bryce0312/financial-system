...
export function StatCard({ label, value, hint }: { label: string; value: string; hint?: string }) {
  const compactClass =
    value.length >= 10
      ? "doodle-stat-card__value doodle-stat-card__value--xcompact"
      : value.length >= 7
        ? "doodle-stat-card__value doodle-stat-card__value--compact"
        : "doodle-stat-card__value";
  ...
  return (
    <Card className="doodle-stat-card rounded-[30px] border-[#ddd6e8] bg-[linear-gradient(180deg,rgba(255,252,247,0.98),rgba(255,255,255,0.98))] shadow-[0_18px_38px_rgba(91,83,111,0.08)]">
      ...
      <CardContent className="doodle-stat-card__body">
        <div className={compactClass}>{value}</div>
